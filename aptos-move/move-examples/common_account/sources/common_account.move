/// A framework for sharing a single nesource account across multiple accounts.
///
...
module common_account::common_account {
    ...
    /// Contains the framework for managing the account, partiuclarly around adminstration.
    ...
    /// Remove the other account to the list of accounts eligible to claim a capability for this
    /// common_account if it exists or remove the capability.
    ...
    public fun acquire_signer(
        sender: &signer,
        common_account: address,
    ): signer acquires Capability, CommonAccount {
        let sender_addr = signer::address_of(sender);
        assert!(exists<Capability>(sender_addr), error::not_found(ENO_CAPABILITY_FOUND));
    ...
    inline fun assert_is_admin(sender: &signer, common_account: address): &mut Management {
        let management = borrow_management(common_account);
        assert!(
            signer::address_of(sender) == management.admin,
    ...
    public fun test_no_account_signer(
        alice: &signer,
        bob: &signer,
    ) acquires Capability, CommonAccount {
        let alice_addr = signer::address_of(alice);
        let common_addr = account::create_resource_address(&alice_addr, vector::empty());
